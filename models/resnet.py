...
from utils.layer_utils import batch_normalization, relu, conv_layer, drop_out, average_pool, fc_layer, max_pool, \
...
class ResNet(BaseModel):
    ...
    def build_network(self, x):
        ...
        with tf.variable_scope('ResNet'):
            net = conv_layer(x, num_filters=32, kernel_size=7, stride=2, add_reg=self.conf.use_reg, layer_name='CONV0')
            net = relu(batch_normalization(net, training=self.is_train_pl, scope='BN1'))
            ...
            net = flatten(net)
            self.features = net
            self.logits = fc_layer(net, num_units=self.conf.num_cls, add_reg=self.conf.use_reg, layer_name='Fc1')
            # [?, num_cls]
    ...
    def bottleneck_block(self, input_x, group, name, is_first_block=False):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('conv_in'):
                ...
                conv = conv_layer(input_x, num_filters=group.bottleneck_size, add_reg=self.conf.use_reg,
                                  kernel_size=1, stride=stride, layer_name='CONV')
                conv = relu(batch_normalization(conv, self.is_train_pl, scope='BN'))

            with tf.variable_scope('conv_bottleneck'):
                conv = conv_layer(conv, num_filters=group.bottleneck_size, kernel_size=3,
                                  add_reg=self.conf.use_reg, layer_name='CONV')
                conv = relu(batch_normalization(conv, self.is_train_pl, scope='BN'))
            ...
            with tf.variable_scope('conv_out'):
                conv = conv_layer(conv, num_filters=group.out_filters, kernel_size=1,
                                  add_reg=self.conf.use_reg, layer_name='CONV')
                conv = batch_normalization(conv, self.is_train_pl, scope='BN')
            ...
            with tf.variable_scope('shortcut'):
                if is_first_block:
                    shortcut = conv_layer(input_x, num_filters=group.out_filters, stride=2, kernel_size=1,
                                          add_reg=self.conf.use_reg, layer_name='CONV_shortcut')
                    shortcut = batch_normalization(shortcut, self.is_train_pl, scope='BN_shortcut')
